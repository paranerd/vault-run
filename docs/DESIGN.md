...
Die linke Spalte trug bis eben eine große Stat-Kachel mit je einer Rate. Sie beschrieb den Durchsatz des Reiches — eine Zahl, die man gegen die des Nachbarabschnitts liest, während der Blick beim Spielen auf dem Button liegt. Jetzt steht dort das Sprite, das vorher auf dem Aktions-Button saß, ohne Kachel und ohne Rahmen, und darunter das, worauf man tatsächlich handelt:

- **Truhe:** die Schatztruhe in ihrer aktuellen Ausbaustufe, darunter **Kapazität** und **Füllstand**.
- **Lager:** der Erzhaufen in seiner aktuellen Ausbaustufe, darunter ebenfalls **Kapazität** und **Füllstand**. Nebeneinander gelesen sagen beide Paare sofort, wo es klemmt — ein volles Lager vor einer halb leeren Truhe heißt: fahren.
...
Der Balken über dem Abschnitt zeigt unverändert seinen einen Fehlermodus — Risiko, Lagerfüllung, Erschöpfung —, und die Zeile unter dem Button die Zahl, die zum Knopfdruck gehört: **Sichtweite**, **Ladung**, **Fördermenge**. Das Risiko startet bei 0 %, steigt, solange Gold in der Schatztruhe liegt, und löst bei 100 % den Diebeszug aus; eine Sicherung drückt es wieder herunter. Je voller der Hort, desto schneller steigt es.

Auf den Aktions-Buttons steht deshalb die **Handlung**, nicht mehr der Gegenstand: die Pickhacke in der Mine (die dort schon immer die Handlung war), ein Goldsack mit Richtungspfeil für „Gold transportieren“ und ein Wächter mit Laterne für „Wache gehen“. Die beiden neuen Icons sind vorerst Platzhalter aus `scripts/generate-sprites.py` und werden später durch gezeichnete ersetzt.

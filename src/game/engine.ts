...
/** Gold, das bereits zur Erzkammer unterwegs ist und dort schon Platz beansprucht. Das Gegenstück zu
...
/** Was noch in die Erzkammer passt. Ist hier nichts mehr frei, ruht die Mine — die Kammer ist die Grenze
 ...
 *  Kammer, die in einer halben Sekunde voll ist, und jeder Schlag, dessen Gold bei der Ankunft
...
/** Schickt frisch geschlagenes Gold auf den Weg in die Erzkammer. Es kommt `GOLD_FLIGHT_DURATION_MS`
 ...
 *  wurde, und der Überlauf entsteht am Fels, wo er nicht mehr in die Kammer passt. Beides ist in
...
/** Legt an `cursor` fällige Förderungen in der Erzkammer ab. Der Deckel greift nur gegen die Unschärfe der
...
/** Der Spieler sichert gerade selbst, ohne dass eine Wache die Arbeit ohnehin täte. Das ist eine
    Aussage über **ihn** und hält nichts im Reich an — Bergleute und Transporte arbeiten währenddessen
...
/** Der Spieler ist eine Person mit zwei Händen. Jede seiner Aktionen belegt ihn für ihre Dauer,
    und solange sie läuft, sind die beiden anderen gesperrt: Wer das Gold ans Tageslicht trägt, steht
 ...
 *  von der Aktion, die läuft: der eigenen Ausfahrt und der Sicherung von Hand. Das sagt ausschließlich
...
/** Gold, das bereits unterwegs ist und die Truhe darum schon beansprucht — die eigene Ausfahrt und
    alle Transporte zusammen. Ohne diese Reservierung packten mehrere gleichzeitig fahrende
    Transporte zusammen mehr ein, als am Ziel noch hineinpasst. */
...
/** Die eigene Ausfahrt. Sie trägt, was in den Beutel des Spielers passt, und braucht die Zeit, die
    seine Stiefel brauchen — beides unabhängig davon, wie viele Transporte gerade unterwegs sind. */
...
/** Historischer Name der eigenen Ausfahrt, solange schon Transporte fahren. Sie ist dieselbe Fahrt —
...
/** Schickt jeden Transport los, der gerade nicht unterwegs ist und etwas zu laden findet. */
 ...
 *
 *  Ist die Erzkammer voll, ruht die Mine bis zur nächsten Ausfahrt: Ein Bergmann, der weiterschlüge,
 *  förderte ausschließlich in den Verlust — über eine Nacht hinweg ein Vielfaches der Kammer an
 *  Gold, das nie irgendwo ankommt. Die letzte Förderung füllt die Kammer noch bis zum Rand auf;
...
function runMiners(state: GameState, cursor: number, report?: OfflineReport): void {
  for (const index of SLOTS) {
    ...
    // Wer neu anfängt — frisch angestellt oder nach der Ruhe an der vollen Kammer —, hängt sich in den
    // laufenden Takt der Uhr ein, statt seinen eigenen bei der Sekunde des Wiederanlaufs zu
    // beginnen. Sonst hinge die Phase daran, wann der Tick die freie Kammer bemerkt: Zusehen
    ...
    while (cursor >= (state.minerBeats[index] as number) + interval) {
      // Füllt eine frühere Förderung dieses Durchlaufs die Kammer, ruht auch der Rest der Takte —
...
/** Ein Bergmann schickt nur ganze Goldstücke in die Erzkammer. Fördert er 0,7 je Takt, kommt im
    ...
    Kammer keine Kommazahl mehr. Der Rest steht im Zustand und übersteht damit Pause und
...
/** Die Mine bringt in diesem Tick nichts hervor: Entweder ist kein Bergmann angestellt, oder der
    volle Kammer hat alle zur Ruhe gebracht. Die zweite Form zählt erst, wenn die Takte tatsächlich
...
/** Ruhezustand: In diesem Tick kann sich am sichtbaren Zustand nichts ändern — keine fördernde
    Mine, keine Fahrt unterwegs oder startbereit, keine laufende Sicherung, kein Risiko und kein
    Gold in der Truhe, das Risiko erzeugen könnte. Das ist das frühe Spiel zwischen zwei Klicks —
    und die volle Kammer, die auf die erste Ausfahrt wartet. */
...
export function advanceGame(input: GameState, now = Date.now(), offline = false): GameState {
  ...
  // die ganze Nacht über unantastbar, egal wie viel die Transporte noch anliefern.
  ...
  while (cursor < target) {
    // Was der Spieler von Hand tut, hält keine Automatik an: Bergleute fördern und Transporte
    // fahren, während er selbst ausfährt oder Wache steht. Angestellte legen die Arbeit
...
export function buySlotUpgrade(state: GameState, group: SlotGroup, index: SlotIndex): GameState {
  ...
  levels[index] += 1
  addEvent(next, 'info', `${group === 'miners' ? 'Bergmann' : group === 'transporters' ? 'Transport' : 'Wache'} ${index + 1} verbessert.`)
  return next
}
